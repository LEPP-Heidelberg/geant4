...
UCNMaterialBoundary2::UCNMaterialBoundary2(const G4String& processName, G4ProcessType type) // chopperFile added by Husain Manasawala on 30.03.2026 to enable reading in the opening function 
         : G4VDiscreteProcess(processName, type)
{
 ...
 fChopperFile = "chopperfunction.dat";
 std::ifstream test("chopperfunction.dat");
 if (test.good()) {
     LoadAndBuildCDF("chopperfunction.dat");
 } else {
     G4cout << "[UCNMaterialBoundary2] No default chopperfunction.dat found, "
            << "This is fine if set using /mat/chopperFile in macro" << G4endl;
 } 
 // End Chopper Logic
...
// Chopper Logic 
void UCNMaterialBoundary2::LoadAndBuildCDF(const G4String& filename)
{
    // ---- 1. Read two-column file (time_ms, voltage) ----
    std::ifstream f(filename);
    if (!f.is_open())
        G4Exception("UCNMaterialBoundary2::LoadAndBuildCDF", "UCN001",
                    FatalException, ("Cannot open chopper file: " + filename).c_str());

    std::vector<G4double> t_ms, v_raw;
    G4double t, v;
    while (f >> t >> v) {
        t_ms.push_back(t);
        v_raw.push_back(v);
    ...
    if (N < 2)
        G4Exception("UCNMaterialBoundaryProcess::LoadAndBuildCDF", "UCN002",
                    FatalException, "Chopper file has fewer than 2 data points.");

    // ---- 2. Shift voltage baseline to zero (mirrors np.maximum(v - min, 0)) ----
    G4double v_min = *std::min_element(v_raw.begin(), v_raw.end());
    std::vector<G4double> v_phys(N);
    for (std::size_t i = 0; i < N; ++i)
        v_phys[i] = std::max(v_raw[i] - v_min, 0.0);

    // ---- 3. Centre of mass in ms, then convert to seconds ----
    G4double wsum = 0.0, tsum = 0.0;
    for (std::size_t i = 0; i < N; ++i) { wsum += v_phys[i]; tsum += t_ms[i] * v_phys[i]; }
    G4double center_ms = tsum / wsum;

    std::vector<G4double> t_ns(N);           // centered, in nanoseconds
    for (std::size_t i = 0; i < N; ++i)
        t_ns[i] = (t_ms[i] - center_ms) * ms;

    // ---- 4. Build PDF via trapezoidal integral, then cumsum → CDF ----
    //  trapz integral
    G4double integral = 0.0;
    for (std::size_t i = 1; i < N; ++i)
        integral += 0.5 * (v_phys[i] + v_phys[i-1]) * (t_ns[i] - t_ns[i-1]);

    std::vector<G4double> pdf(N);
    for (std::size_t i = 0; i < N; ++i)
        pdf[i] = v_phys[i] / integral;

    //  cumulative sum of pdf * dt  (trapezoidal, so it matches np.cumsum on the pdf array)
    fCDF_values.resize(N);
    fCDF_times .resize(N);
    fCDF_values[0] = 0.0;
    for (std::size_t i = 1; i < N; ++i)
        fCDF_values[i] = fCDF_values[i-1]
                         + 0.5 * (pdf[i] + pdf[i-1]) * (t_ns[i] - t_ns[i-1]);

    // ---- 5. Normalise CDF to [0,1] exactly and store matching times ----
    G4double cdf_max = fCDF_values.back();
    for (std::size_t i = 0; i < N; ++i) {
        fCDF_values[i] /= cdf_max;
        fCDF_times [i]  = t_ns[i];
    }

    G4cout << "[UCNMaterialBoundaryProcess] Chopper CDF built from "
	   << fChopperFile << ", containing " 
           << N << " points. t range: ["
           << fCDF_times.front() << ", " << fCDF_times.back()
           << "] s  (centred on 0)" << G4endl;

        /* // Debugging
        #include <fstream>

	// Draw 100,000 samples and bin them manually
	const int NBINS = 100;
	const int NSAMPLES = 100000;
	std::vector<int> counts(NBINS, 0);
	G4double t_min = fCDF_times.front();
	G4double t_max = fCDF_times.back();
	G4double bin_width = (t_max - t_min) / NBINS;

	G4cout << "[DEBUG] First 5 CDF values: " << G4endl;
	for(int i = 0; i < 5; ++i)
	    G4cout << "  t=" << fCDF_times[i] << "  cdf=" << fCDF_values[i] << G4endl;
	G4cout << "[DEBUG] Last 5 CDF values: " << G4endl;
	for(int i = fCDF_values.size()-5; i < (int)fCDF_values.size(); ++i)
	    G4cout << "  t=" << fCDF_times[i] << "  cdf=" << fCDF_values[i] << G4endl;

	G4cout << "[DEBUG] 10 raw SampleJitter() calls:" << G4endl;
	for(int i = 0; i < 10; ++i)
	    G4cout << "  " << SampleSmear() << G4endl;

	for (int i = 0; i < NSAMPLES; ++i) {
	    G4double t = SampleSmear();
	    int bin = (int)((t - t_min) / bin_width);
	    if (bin >= 0 && bin < NBINS) counts[bin]++;
	}

	std::ofstream out("jitter_check.csv");
	out << "time_s,count\n";
	for (int i = 0; i < NBINS; ++i) {
	    G4double bin_centre = t_min + (i + 0.5) * bin_width;
	    out << bin_centre << "," << counts[i] << "\n";
	}
	out.close();
	G4cout << "[UCNMaterialBoundary2] Wrote jitter_check.csv" << G4endl; */ 
}

G4double UCNMaterialBoundary2::SampleSmear() const
{
    if (fCDF_values.empty()) {
        G4Exception("UCNMaterialBoundary2::SampleSmear", "UCN003",
        FatalException, "Chopper file not loaded — use /mat/chopperFile in mac");
        }
    	
    // Draw a uniform random number in [0,1) using GEANT4's engine
    G4double u = G4UniformRand();

    // Binary search for the bracket in the CDF (fCDF_values is sorted ascending)
    auto it = std::lower_bound(fCDF_values.begin(), fCDF_values.end(), u);

    // Handle edge cases
    if (it == fCDF_values.begin()) return fCDF_times.front();
    if (it == fCDF_values.end())   return fCDF_times.back();

    std::size_t hi = std::distance(fCDF_values.begin(), it);
    std::size_t lo = hi - 1;

    // Linear interpolation (inverse CDF)
    G4double cdf_lo = fCDF_values[lo], cdf_hi = fCDF_values[hi];
    G4double t_lo   = fCDF_times [lo], t_hi   = fCDF_times [hi];

    G4double frac = (u - cdf_lo) / (cdf_hi - cdf_lo);
    return t_lo + frac * (t_hi - t_lo);   // result in seconds
}
// End Chopper Logic 
...
//	G4cout << "UCNMaterialBoundary2 record first steps" << aTrack.GetCurrentStepNumber() << ", " << stepsbeforekill <<  G4endl;
  ...
  myfile << aTrack.GetKineticEnergy()/neV << " " << aTrack.GetMomentumDirection().getX() << " " <<  aTrack.GetMomentumDirection().getY() << " " <<
                  ...
                  chopper_t + chopper_smear << " " << chopper_t << " " << chopper_x << " " << foilct << " " << chopper_z << " " << chopper_px << " " << chopper_py << " " << "10000" << std::endl;
 ...
 G4cout << "Reached defined max age: " << maxGlobalTime << "s, at Step Number: " << aTrack.GetCurrentStepNumber() << ", global time "
  ...
  aParticleChange.Initialize(aTrack);
      ...
      std::ofstream myfile(outputfile1, std::ofstream::app);
          myfile << aTrack.GetKineticEnergy()/neV << " " << aTrack.GetMomentumDirection().getX() << " " <<  aTrack.GetMomentumDirection().getY() << " " <<
                  aTrack.GetMomentumDirection().getZ() << " " << pPreStepPoint->GetPosition().getX() << " " << pPreStepPoint->GetPosition().getY() << " " << pPreStepPoint->GetPosition().getZ() << " " << aTrack.GetGlobalTime() << " " <<
                  chopper_t + chopper_smear << " " << chopper_t << " " << chopper_x << " " << foilct << " " << chopper_z << " " << chopper_px << " " << chopper_py << " " << "13000" << std::endl;
...
G4cout << "Killed pp foil" << foiltrans << ", rnd=  " << counted << G4endl;
	...
	std::ofstream myfile(outputfile1, std::ofstream::app);
          myfile << aTrack.GetKineticEnergy()/neV << " " << aTrack.GetMomentumDirection().getX() << " " <<  aTrack.GetMomentumDirection().getY() << " " <<
                  aTrack.GetMomentumDirection().getZ() << " " << pPreStepPoint->GetPosition().getX() << " " << pPreStepPoint->GetPosition().getY() << " " << pPreStepPoint->GetPosition().getZ() << " " << aTrack.GetGlobalTime() << " " <<
                  chopper_t + chopper_smear << " " << chopper_t << " " << chopper_x << " " << foilct << " " << chopper_z << " " << chopper_px << " " << chopper_py << " " << "1000" << std::endl;
...
G4cout << "Falling down" << G4endl;
          ...
          myfile << aTrack.GetKineticEnergy()/neV << " " << aTrack.GetMomentumDirection().getX() << " " <<  aTrack.GetMomentumDirection().getY() << " " << 
                  aTrack.GetMomentumDirection().getZ() << " " << pPreStepPoint->GetPosition().getX() << " " << pPreStepPoint->GetPosition().getY() << " " << pPreStepPoint->GetPosition().getZ() << " " << aTrack.GetGlobalTime() << " " <<
                  chopper_t + chopper_smear << " " << chopper_t << " " << chopper_x << " " << foilct << " " << chopper_z << " " << chopper_px << " " << chopper_py << " " << "15000" << std::endl;
...
myfile << aTrack.GetKineticEnergy()/neV << " " << aTrack.GetMomentumDirection().getX() << " " <<  aTrack.GetMomentumDirection().getY() << " " <<  aTrack.GetMomentumDirection().getZ() << " " << pPreStepPoint->GetPosition().getX() << " " << pPreStepPoint->GetPosition().getY() << " " << pPreStepPoint->GetPosition().getZ() << " " << aTrack.GetGlobalTime() <<" " <<  chopper_t + chopper_smear << " " << chopper_t << " " << chopper_x << " " << foilct << " " << chopper_z << " " << chopper_px << " " << chopper_py << " " << "12000" << std::endl;
...
std::ofstream myfile(outputfile1, std::ofstream::app);
myfile << aTrack.GetKineticEnergy()/neV << " " << aTrack.GetMomentumDirection().getX() << " " <<  aTrack.GetMomentumDirection().getY() << " " <<  aTrack.GetMomentumDirection().getZ() << " " << pPreStepPoint->GetPosition().getX() << " " << pPreStepPoint->GetPosition().getY() << " " << pPreStepPoint->GetPosition().getZ() << " " << aTrack.GetGlobalTime() <<" " <<  chopper_t + chopper_smear << " " << chopper_t << " " << chopper_x << " " << foilct << " " << chopper_z << " " << chopper_px << " " << chopper_py << " " << "8000" << std::endl;
...
if (counted < beff){
	std::ofstream myfile(outputfile1, std::ofstream::app);
	  myfile << aTrack.GetKineticEnergy()/neV << " " << aTrack.GetMomentumDirection().getX() << " " <<  aTrack.GetMomentumDirection().getY() << " " <<  
		  aTrack.GetMomentumDirection().getZ() << " " << pPreStepPoint->GetPosition().getX() << " " << pPreStepPoint->GetPosition().getY() << " " << pPreStepPoint->GetPosition().getZ() << " " << aTrack.GetGlobalTime() << " " <<  
		  chopper_t + chopper_smear << " " << chopper_t << " " << chopper_x << " " << foilct << " " << chopper_z << " " << chopper_px << " " << chopper_py << " " << "6000" << std::endl;
...
std::ofstream myfile(outputfile1, std::ofstream::app);
	myfile << aTrack.GetKineticEnergy()/neV << " " << aTrack.GetMomentumDirection().getX() << " " <<  aTrack.GetMomentumDirection().getY() << " " <<  aTrack.GetMomentumDirection().getZ() << " " << pPreStepPoint->GetPosition().getX() << " " << pPreStepPoint->GetPosition().getY() << " " << pPreStepPoint->GetPosition().getZ() << " " << aTrack.GetGlobalTime() <<" " <<  chopper_t + chopper_smear << " " << chopper_t << " " << chopper_x << " " << foilct << " " << chopper_z << " " << chopper_px << " " << chopper_py << " " << "7000" << std::endl;
}
	  aParticleChange.ProposeTrackStatus( fStopAndKill ) ;
 return G4VDiscreteProcess::PostStepDoIt(aTrack, aStep);
...
//G4cout << " material1 " << Material1 << ", mart2 " << Material2 << ", reflection = " << reflection << G4endl;
         ...
         // If NOT in chopper, this must be commented out for vTOF!
	 ...
	 myfile << aTrack.GetKineticEnergy()/neV << " " << aTrack.GetMomentumDirection().getX() << " " <<  aTrack.GetMomentumDirection().getY() << " " <<  aTrack.GetMomentumDirection().getZ() << " " << pPreStepPoint->GetPosition().getX() << " " << pPreStepPoint->GetPosition().getY() << " " << pPreStepPoint->GetPosition().getZ() << " " << aTrack.GetGlobalTime() <<" " <<  chopper_t + chopper_smear << " " << chopper_t << " " << chopper_x << " " << foilct << " " << chopper_z << " " << chopper_px << " " << chopper_py << " " << "5000" << std::endl;
          ...
          // END SANITY CHECK ----------------------------------------------- 
	*/
        if ( fermipot < 0  ){ // Comment out this if block for sanity checks.
...
std::ofstream myfile(outputfile1, std::ofstream::app);
  ...
  myfile << "# -------------------------------------------------------------------------------------------"<<std::endl;
  myfile << "# KE[neV] px[unitless] py[unitless] pz[unitless] x[mm] y[mm] z[mm] t_global[ns] t_chopper_smeared[ns] t_chopper_raw[ns] x_chopper[mm] n_foil z_chopper[mm] px_chopper[unitless] py_chopper[unitless] Exit Code" << std::endl;
